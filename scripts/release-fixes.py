...
# They must be visible/usable even when the recipient is not Pro.
replacements = {
...
viewer_replacement = r'''function FullScreenVideoViewer({ videoId, ar, onClose }) {
  ...
  const tikTokUrl = /^\d+$/.test(raw)
    ? `https://www.tiktok.com/video/${raw}`
    : raw;
  ...
  return (
    ...
    >
      <div style={{
        ...
        justifyContent: "space-between", padding: "10px 12px", gap: 8,
      }}>
...
function extractTikTokVideoId'''
text, count = viewer_pattern.subn(viewer_replacement, text, count=1)
if count != 1:
    raise SystemExit(f"FullScreenVideoViewer block not found: {count}")

# Cardio completion is the same completion contract as normal exercises.
# Finish/skip must persist finished=true and clear the start timestamp so the
# timestamp-based timer can never resume after the user has finished.
...
# Preserve custom plan visibility in Plans/Workout for non-Pro recipients.
text = text.replace('data.customTrainingPlan && data.entitlements.trainingPro && (', 'data.customTrainingPlan && (')
text = text.replace('data.customNutritionPlan && data.entitlements.nutritionPro && (', 'data.customNutritionPlan && (')
text = text.replace(
...
print("Release fixes applied successfully")
