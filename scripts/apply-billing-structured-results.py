...
# launchBillingFlow's PurchasesUpdatedListener must always return a structured
# result for non-OK responses. Do not let Capacitor turn the numeric code into a
...
# The synchronous launch result is authoritative for launch failures. Return it
# as data so JavaScript always sees responseCode even when Capacitor reject()
# serialization differs across versions.
text = text.replace(
...
# Keep querySkuDetails behavior unchanged; these replacements are intentionally
# limited to the launchBillingFlow method below its declaration.
