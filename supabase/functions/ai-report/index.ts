...
Deno.serve(async (req) => {
  ...
  const responseText = typeof body.response === "string" ? body.response.trim().slice(0, 2000) : "";
  const reason = typeof body.reason === "string" ? body.reason.trim().slice(0, 500) : "";
  const lang = body.lang === "ar" ? "ar" : "en";
  const model = typeof body.model === "string" ? body.model.trim().slice(0, 120) : null;

  if (!responseText || !reason) {
    return json(400, { error: "bad_request", message: "response and reason are required" });
  }
  ...
  if (countError) return json(500, { error: "backend_error" });
  ...
  const { error } = await sb.from("ai_reports").insert({
    uid,
