...
# The source-hardening Vite plugin historically required the native Browser flow.
# Its other production checks remain useful, but its TikTok check must now accept
# the in-app iframe viewer created above.
VITE = Path('vite.config.js')
v = VITE.read_text(encoding='utf-8')
old_check = "videoSegment.includes('Browser.open({ url: tikTokUrl')"
if old_check not in v:
    raise SystemExit('Vite TikTok hardening check not found')
v = v.replace(old_check, "videoSegment.includes('<iframe')", 1)
v = v.replace('TikTok native browser flow', 'TikTok in-app iframe flow', 1)
VITE.write_text(v, encoding='utf-8')

print('Release fixes v3 applied successfully')
