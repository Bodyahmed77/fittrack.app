...
async function prepareChannel() {
  ...
  try {
    ...
    // Channel creation is best-effort; scheduling can still work with the app default channel.
  }
  try {
    ...
    // Permission prompts are platform controlled; do not block app startup.
  }
  return true;
}

function isRecentPlanNotification(data) {
  const raw = data?.createdAt || data?.timestamp || data?.publishedAt;
  if (!raw) return true;
    ...
    unsubscribeFirestore = onSnapshot(
      ...
      (snapshot) => {
        snapshot.docChanges().forEach((change) => {
          ...
          seenNotificationIds.add(id);
          const isPlan = ["nutrition_plan_ready", "training_plan_ready"].includes(String(data.type || ""));
          if (isPlan || isRecentPlanNotification(data)) {
