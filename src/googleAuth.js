...
// Must stay under 60s so the UI never sticks on "Opening Google…" forever.
const GOOGLE_SIGNIN_TIMEOUT_MS = 60000;
...
function mapNativeError(e) {
  ...
  if (
    /\b10\b/.test(raw) ||
    /DEVELOPER_ERROR/i.test(combined) ||
    /ApiException:\s*10/i.test(raw) ||
    /\[28444\]|Developer console is not set up/i.test(combined)
  ) {
    const err = new Error(
  ...
  if (/12501|SIGN_IN_CANCELLED|canceled|cancelled|USER_CANCELLED/i.test(combined)) {
    const err = new Error("Google Sign-In was cancelled");
    err.code = "cancelled";
    return err;
  }
  if (/12500|SIGN_IN_FAILED/i.test(combined)) {
  ...
  if (/network|NETWORK_ERROR|7\b/i.test(combined) && /ApiException|Google/i.test(combined)) {
    const err = new Error("Network error during Google Sign-In");
    err.code = "network";
    return err;
  }
...
async function nativeGoogleIdToken() {
  console.info("[GoogleSignIn] path=native start");
  ...
  console.info(
    "[GoogleSignIn] plugin loaded; calling signInWithGoogle(useCredentialManager:false, skipNativeAuth:true)",
  );

  try {
    if (typeof FirebaseAuthentication.signOut === "function") {
      await FirebaseAuthentication.signOut();
      console.info("[GoogleSignIn] native signOut before signIn (ok)");
    }
  } catch (pre) {
    console.info(
      "[GoogleSignIn] native signOut before signIn skipped",
      String(pre?.message || pre).slice(0, 80),
    );
  }
...
async function webGoogleSignIn() {
  console.info("[GoogleSignIn] path=web popup");
