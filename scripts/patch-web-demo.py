...
def patch_main():
    ...
    s = s.replace('const FIFTYFIT_WEB_DEMO_V1 =', '/* FIFTYFIT_WEB_DEMO_V1 */\nconst FIFTYFIT_WEB_DEMO_V1 =', 1)
...
def patch_app():
    ...
    new_phase = '''    if (firebaseUser === null) {\n      setPhase("welcome");\n      return;\n    }\n    if (typeof window !== "undefined" && window.__FIFTYFIT_DEMO_MODE__ && loaded) {\n      // Ad-preview web build: use the real app shell/data model but skip\n      // account/onboarding screens so the reviewer can explore immediately.\n      setPhase("app");\n      return;\n    }\n    if (!loaded || writePending) return;'''
    ...
    # Stable web-demo diagnostics + no horizontal scrolling even where older
    # inline styles were authored before responsive hardening.
    marker = '/* FIFTYFIT_WEB_DEMO_APP_V1 */\n'
    s = marker + s
